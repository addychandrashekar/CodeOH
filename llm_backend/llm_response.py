...
def generate_test_cases(file_content, filename):
    """
    Generates test cases for methods or functions in a given file.

    Args:
        file_content (str): The content of the file to generate test cases for
        filename (str): The name of the file

    Returns:
        dict: The generated test cases with explanations
    """
    prompt = f"""
    You are an expert test writer inside a developer's code editor. Given the following code file, generate comprehensive test cases for each method or function in the file.
    ...
    ### Instructions for Response:
    - For each method/function in the file, create a corresponding test function/method
    - Follow the appropriate testing framework and conventions based on the language:
    ...
    - Write comprehensive tests that:
      * Test normal input/output behavior
      * Test edge cases 
      * Test error conditions
      * Include appropriate assertions
    - Include setup and teardown when necessary
    - Organize tests logically and provide clear test names
    - Add brief comments explaining what each test is checking
    - Make your tests deterministic and isolated
    - Return the tests as a complete file that can be saved and run
    - Suggest a filename for the test file following conventions
    
    ### Response Format:
    - Start with a suggested filename for the test file
    - Include all necessary imports/dependencies
    - Include complete, runnable test code
    - Use proper test syntax and formatting for the language
    """

    print(f"Test generation prompt length: {len(prompt)}")
    ...
    # Extract suggested test filename
    test_filename = None
    filename_pattern = r'filename:?\s+["\']?([^"\'\n]+)["\']?'
    match = re.search(filename_pattern, text, re.IGNORECASE)
    if match:
        test_filename = match.group(1).strip()

    # If no suggested filename found, create one based on conventions
    ...
    return {
        "text": text,
        "file_data": {"filename": test_filename, "content": text, "is_new_file": True},
    }
